...
ON_COMMAND(ID_TRANSPORTER_ADDTRANSPORTER, &CChildView::OnTransporterAddtransporter)
ON_UPDATE_COMMAND_UI(ID_TRANSPORTER_TRANSPORT, &CChildView::OnUpdateTransporterTransport)
ON_COMMAND(ID_TRANSPORTER_TRANSPORT, &CChildView::OnTransporterTransport)
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    if (mGrabbedItem != nullptr)
    {
        ...
        //if the transporters are active
        if (mTransport) 
        {
            //check if the grabbed tile is a transporter
            CTransporterCounter counterVisitor;
            mGrabbedItem->Accept(&counterVisitor);
            bool readyToTransport = counterVisitor.CanTransport();
            if (readyToTransport)
            {
                //reset the counter from the tile and send the visitor to all tranporters in the city
                counterVisitor.Reset();
                mCity.Accept(&counterVisitor);
                //if there are multiple transporters
                if (counterVisitor.GetNumTransporters() > 1)
                { 
                    //get minion location
                    int minionLoc = counterVisitor.GetMinionLocation();
                    int destLoc = 0;
                    //if the tile has no minion, set destination to last tile clicked
                    if (!((CTileTransporter*)(mGrabbedItem.get()))->HasMinion())
                    {

                        destLoc = (counterVisitor.GetNumTransporters() - 1);
                    }
                    //if tile has minion, set random location
                    //add 1 to skip minion location
                    else
                    {
                        destLoc = rand() % (counterVisitor.GetNumTransporters() - 1);
                        if (destLoc >= minionLoc)
                        {
                            destLoc++;
                        }
                    }
                    //set from and destination locations and move minion
                    CMinionMover visitor;
                    visitor.SetFromDestLocs(minionLoc, destLoc);
...
/**
* Add transporter to city
*/
void CChildView::OnTransporterAddtransporter()
{
    CTransporterCounter visitor;
    mCity.Accept(&visitor);
    auto tile = make_shared<CTileTransporter>(&mCity);
    tile->SetLocation(InitialX, InitialY);
    mCity.Add(tile);
    if (visitor.GetNumTransporters() == 0)
    {
        tile->AddMinion();
    }
    Invalidate();
...
/**
*turn on mtransporters
*/
void CChildView::OnTransporterTransport()
{
    mTransport = !mTransport;
}

/**
*turn on transporters
*\param pCmdUI command update
*/
void CChildView::OnUpdateTransporterTransport(CCmdUI* pCmdUI)
{
    pCmdUI->SetCheck(mTransport);	// Will check the menu option
}
...
{
    pCmdUI->SetCheck(mShowHistory);	// Will check the menu option
}